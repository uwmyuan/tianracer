...
from tianracer_platoon.cfg import tf_pidConfig  # 修改这里，使用tianracer_platoon包的配置
import numpy as np
...
def odom_cb(msg):
    global w_leader, v_leader
    w_leader = msg.twist.twist.angular.z
    v_leader = msg.twist.twist.linear.x
    

if __name__ == '__main__':
    rospy.init_node('tianracer_tf_listener')  # 修改节点名称为tianracer_tf_listener
    ...
    cmd_publisher = rospy.Publisher(
        follower_robot_name + '/ackermann_cmd_stamped', AckermannDriveStamped, queue_size=1
    )  
    ...
    while not rospy.is_shutdown():
        ...
        WB = 0.26
        steer = np.math.atan(w_follower * WB / v_follower)
        ack = AckermannDriveStamped()
        ack.header.stamp = rospy.Time.now()
        ack.drive.speed = float(v_follower)
        ack.drive.steering_angle = float(steer)
        cmd_publisher.publish(ack)
